			...
			if (!isset($errormsg) && isset($api->connect) && $api->connect == TRUE) {
				...
				*/
				if ($api->data->status == 1) {
					$nick = $_SESSION['user'];
					$przedmiot = "VIP"; // EDYTUJ NAZWĘ USŁUGI JAK DODAJESZ NOWĄ
					$data = date("d/m/Y H:i");
					...
					if ($rcon->connect()){
					  $rcon->sendCommand("pex user $nick group add VIP * 2419200");
					  $rcon->sendCommand("broadcast $nick zakupil range &6VIP. &aDziękujemy!");
					} else {
...
<div id="page">
     <div class="center">
			 <h2>Kupujesz rangę VIP na okres 28 dni </h2>
